...
setMethod("batchCorrect", "NothingParam", function(..., batch = NULL, 
    ...
    get.spikes = FALSE, PARAM) 
{
    batches <- list(...)

    # Pulling out information from the SCE objects.        
    if (batchelor:::.check_if_SCEs(batches)) {
        batchelor:::.check_batch_consistency(batches, byrow=TRUE)
        batchelor:::.check_spike_consistency(batches)
        subset.row <- batchelor:::.SCE_subset_genes(subset.row, batches[[1]], get.spikes)

        if (is.null(assay.type)) assay.type <- "logcounts" # due to default from generic.
    ...
    if (do.split) {
        ...
        divided <- batchelor:::.divide_into_batches(batches[[1]], batch=batch, byrow=FALSE)
