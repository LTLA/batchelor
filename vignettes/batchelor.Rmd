...
vignette: >
  %\VignetteIndexEntry{Correcting batch effects}
...
A single matrix of corrected PC scores is returned, which can be used directly for downstream analyses such as clustering and visualization.

```{r}
f.out <- fastMNN(A=sce1, B=sce2)
dim(f.out$corrected) # rows are cells now.
```
...
dim(f.out2$corrected)
```

As we can see, the batch effect is successfully removed in the PCA plot below.

```{r}
reducedDim(combined, "corrected") <- f.out2$corrected
plotReducedDim(combined, colour_by="batch", use_dimred="corrected")
```
...
```{r}
cor.exp <- tcrossprod(metadata(f.out2)$rotation[1,,drop=FALSE], f.out2$corrected)
hist(cor.exp[1,], xlab="Corrected expression for gene 1") 
```
